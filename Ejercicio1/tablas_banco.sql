...
FROM clients
INNER JOIN accounts 
ON clients.id=accounts.id;

-- ejercicio2
SELECT  accounts.account_no , accounts.client_id  , account_types."name"  
FROM accounts
INNER JOIN account_types 
ON accounts.type=account_types.id;
-- ejercicio3
SELECT transactions.id , transactions.amount , transactions_types."name"  
FROM transactions 
INNER JOIN transactions_types 
ON transactions.transaction_type = transactions_types.id ;

-- ejercicio4
SELECT transactions.id , transactions.amount , transactions_types."name"  
FROM transactions 
right JOIN transactions_types 
ON transactions.transaction_type = transactions_types.id ;
